...
interface ArticleRepositoryInterface
{
     ...
     *
     * 指定したIDの記事が存在する場合はArticleオブジェクトを返す。
     *
     * @param  int $id 取得する記事のID
     * @return Article 記事が存在する場合はArticleオブジェクト。
     *
     ...
     *
     * @param  string|null $keyword   検索に使用するキーワード
     ...
     * 論理削除された記事を元の状態に復元する。
     *
     * 指定された記事が論理削除された状態の場合、その削除済みフラグを解除し、通常の状態に戻す。
     *
     * @param Article $article 復元対象の記事オブジェクト
     *
     * @throws \DomainException 論理削除されていない記事が指定された場合
     * @throws \RuntimeException データベースエラーなど、予期しない例外が発生した場合
     */
    public function restore(Article $article): void;

    /**
     * 指定された記事IDに対応する論理削除された記事を取得。
     *
     * 指定したIDに基づいて論理削除状態にある記事を検索し、存在すればその Article オブジェクトを返す。
     *
     * @param  int $id 記事の一意な識別子
     * @return Article 論理削除された記事が存在する場合は Article オブジェクト。
     *
